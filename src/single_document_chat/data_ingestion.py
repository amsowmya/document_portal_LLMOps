...
class SingleDocIngestor:
    def __init__(self, daata_dir: str = "data/single_document_chat", faiss_dir: str = "faiss_index", session_id=None):
        try:
            self.log = CustomLogger().get_logger(__name__)            
            self.data_dir = Path(daata_dir)
    ...
    def _create_retriever(self, documents):
        try:
            ...
            self.log.info("Retriever created successfully", retreiver_type=str(type(retriever)))
            return retriever
        except Exception as e:
