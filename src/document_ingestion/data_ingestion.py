...
class FaissManager:
    ...
    def load_or_create(self, texts: Optional[List[str]]=None, metadatas: Optional[List[str]] = None):
        if self._exists():
...
class DocumentComparator:
    """
    Save, read and combine PDF's for comparision with session based versioning
    """
    ...
    def save_uploaded_files(self, reference_file, actual_file):
        try:
            ...
            raise DocumentPortalException("Error saving PDF file", e) from e
    ...
    def combine_documents(self):
        try:
            ...
            combined_text = "\n\n".join(doc_parts)
            self.log.info(f"Documents combined", count=len(doc_parts), session_id=self.session_id)
            return combined_text
        except Exception as e:
            self.log.error("Error combinig documents", error=str(e), session_id=self.session_id)
            raise DocumentPortalException(f"Error combining documents: {str(e)}", e)
    ...
    def clean_old_session(self, keep_latest: int = 3):
        try:
            ...
            for folder in sessions[keep_latest:]:
                ...
                self.log.info("Old session folder is deleted", path=str(folder))
        except Exception as e:
            self.log.error("Error cleaning old session", error=str(e), session_id=self.session_id)
            raise DocumentPortalException(f"Error cleaning old session: {str(e)}", e)
